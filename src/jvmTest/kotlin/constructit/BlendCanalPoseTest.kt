...
class BlendCanalPoseTest {
     ...
     * ordinary one, and this class's own claim is what says the march is the body's and not the pose's: it
     * takes what it takes on `XY`, on every plane the drawing can be sketched on.
     */
    @Test
    fun theTightBendCanalTakesTheSameMaterialInEveryPose() {
        assertSameInEveryPose("the tight-bend canal along an elliptical mitre") { pose ->
            val cx = Construction()
            val box = prismOn(pose, cx, listOf(Vec2(0.0, 0.0), Vec2(width, 0.0), Vec2(width, depth), Vec2(0.0, depth)), height)
            val s0 = Evaluator().solid(box)
            val corner = at(pose, width, depth, height)
            ...
            val es = edgesOf(rounded)
            val mitre = assertNotNull(es.indices.firstOrNull { es[it].name is EdgeName.BlendMitre && es[it].reason == null }, "${pose.what}: the two bands cross in a mitre")
            val (out, why) = round(cx, assertNotNull(two), listOf(mitre), 2.5)
            val body = Evaluator().solid(assertNotNull(out, "${pose.what}: the tight-bend canal builds: $why"))
            assertManifold(body.mesh, "${pose.what}: the tight-bend canal")
            before to before - Geom3.volume(body.mesh)
        }
    }
