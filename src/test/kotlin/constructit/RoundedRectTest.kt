...
class RoundedRectTest {
    ...
    private fun bboxExtents(ev: Evaluator, segments: List<constructit.dsl.SegmentRef>): Pair<Double, Double> {
    ...
    fun dimensionsRadiusAndPathIds() {
        ...
        assertClose(w, 40.0); assertClose(h, 30.0)
    ...
    fun standardRectIsRoundedWith2mm() {
        ...
        for (i in std.arcs.indices) {
            ...
            val a = ev.arc(std.arcs[i]); val b = ev.arc(ref.arcs[i])
            assertClose(a.center.x, b.center.x); assertClose(a.center.y, b.center.y)
        }
    }
