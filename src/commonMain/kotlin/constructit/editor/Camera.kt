...
data class Camera(val panX: Double, val panY: Double, val scale: Double) {
    ...
    fun pan(dxScreen: Double, dyScreen: Double) = copy(panX = panX + dxScreen, panY = panY + dyScreen)

    /** Zoom by [factor] while keeping the world point under screen position [s] fixed. */
    fun zoomAt(s: Vec2, factor: Double): Camera {
    ...
    companion object {
        ...
        fun centered(wPx: Double, hPx: Double, scale: Double = 4.0) = Camera(wPx / 2, hPx / 2, scale)
    }
}
