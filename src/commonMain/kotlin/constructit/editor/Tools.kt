...
class Picks(
    ...
    /**
     * The **size law** a variable-section sweep was stated with (OP-26, session 77) — one expression over the
     * run parameter `t`, verbatim as the user wrote it — or null for a section of one size.
     *
     * A *text*, and therefore not a [scalars] slot: a law is not a number, and the tool table's input
     * language is clicks and numbers ([Editor.addFunctionCurve]'s own recorded argument). It rides the step
     * as an **optional** `law=` and absence is the whole of what keeps every file ever written meaning
     * exactly what it meant (OP-18).
     */
    val law: String? = null,
    /**
     * The **family laws** a function-family section was swept with (OP-26, session 79) —
     * semicolon-separated `name = formula` pairs over the run parameter `t`, verbatim as the user wrote
     * them — or null for a section read once as it is drawn.
     *
     * [law]'s general tier and a *text* for the same reason: laws are written, and the tool table's input
     * language is clicks and numbers. It rides the step as an optional `laws=` on the one tool that carries
     * it ([ToolDef.carriesLaws]), and absence is the whole of what keeps every file ever written meaning
     * exactly what it meant (OP-18).
     */
...
class ToolDef(
    ...
    /**
     * Whether this tool carries a **size law** over the run parameter — a variable section (OP-26, session
     * 77). Declared by the two tools whose section has a size to state and by nothing else, so a `law=` on
     * any other step is refused by name at load rather than silently dropped.
     */
    val carriesLaw: Boolean = false,
    /**
     * Whether this tool carries **family laws** over the run parameter — a section read once per station
     * (OP-26, session 79). Declared by the one tool whose section is a *drawing* and by nothing else, so a
     * `laws=` on any other step is refused by name at load rather than silently dropped ([carriesLaw]'s own
     * rule, and for its own reason: a file that says *varying* and builds *constant* is the one thing a load
     * may not do).
     *
     * A tube declares [carriesLaw] and not this: its section is a circle the run itself states, so there is
     * no drawing to read per station and its one size is already `r(t)`.
     */
    val carriesLaws: Boolean = false,
    /**
    ...
    /**
     * The words of a tool that is **not** in the message bundle — a document's own macro (OP-6), whose name
     * and help sentence come from the document rather than from `l10n/app_en.arb`.
     *
     * Null for every tool in the table below, and that is the point of OP-29's arrangement: a built-in
     * tool's [label], [help] and slot names are looked up by its **id**, so the table states the structure
     * and the ARB states the words. See the derived [label] / [help] / [slotNames] properties.
     */
    private val labelText: String? = null,
    private val helpText: String? = null,
    private val slotNameTexts: List<String> = emptyList(),
    /**
    ...
    /**
     * This tool's name in the active language (OP-29): `tool.<id>.title` in the message bundle, or the
     * [labelText] a document's macro carries instead.
     */
    val label: String get() = labelText ?: Messages.text("tool.$id.title")

    /**
     * This tool's help sentence in the active language (OP-29), empty where it declares none — the same
     * `""` the table used to default to, so every reader of it is unchanged.
     */
     ...
     *
     * Optional, and given only where the word is worth more than the slot's kind: the inspector's
     * *built from* row uses it to say `centre e4, radius point e5` instead of listing two anonymous
     * points (see [Dependencies]). [roleOf] falls back to the slot kind's own word, so an undeclared
     * tool still says *something*.
     */
    val slotNames: List<String> get() = if (slotNameTexts.isNotEmpty()) slotNameTexts else declaredSlotNames()

    private fun declaredSlotNames(): List<String> {
        val names = ArrayList<String>()
        while (true) {
            names.add(Messages.patternOrNull("tool.$id.slot.${names.size + 1}") ?: break)
        }
        return names
    }

    /** What slot [i] is for: this tool's own word ([slotNames]) or the slot kind's generic one. */
    fun roleOf(i: Int): String =
        (slotNameTexts.getOrNull(i) ?: Messages.patternOrNull("tool.$id.slot.${i + 1}"))
            ?: Tools.roleOfKind(slots.getOrNull(i))
...
object Tools {
     ...
     *
     * Two rows for the blends' own reason, one granularity down: whether the shell has an **opening** is a
     * *statement*, not a reading of where the click landed. [SHELL] opens the face the click named — the cup,
     * the box, the housing — and [SHELL_CLOSED] leaves the body closed all round, which is a legal shell and
     * the one a printed float or a sealed vessel wants. A single row that inferred which was meant would have
     * to guess from a miss, and the row a gesture used is the most durable thing a step can carry (OP-18).
     */
    const val SHELL = "shell"
    const val SHELL_CLOSED = "shellclosed"
    ...
    val all: List<ToolDef> =
        listOf(
            ...
            ToolDef(POINT, ToolCategory.POINTS, listOf(SlotKind.PLACE_POINT), shortcut = 'P', replicates = false, icon = Icons.POINT) { _, _, _ -> },
            ...
            ToolDef(MIDPOINT, ToolCategory.POINTS, listOf(SlotKind.POINT, SlotKind.POINT), scalars = listOf(choice("factor", 0.5)), icon = Icons.MIDPOINT) { d, p, s -> d.midpoint(p.points[0], p.points[1], s.firstOrNull()) },
            ToolDef(INTERSECT, ToolCategory.POINTS, listOf(SlotKind.CURVE, SlotKind.CURVE), icon = Icons.INTERSECT) { d, p, _ -> d.intersect(p.elements[0], p.elements[1]) },
            ToolDef(PROJECT, ToolCategory.POINTS, listOf(SlotKind.POINT, SlotKind.LINE), icon = Icons.PROJECT) { d, p, _ -> d.projectToLine(p.points[0], p.elements[0]) },
            ...
            ToolDef(POINT_ON_CIRCLE, ToolCategory.POINTS, listOf(SlotKind.CIRCLE), replicates = false, icon = Icons.POINT_ON_CIRCLE) { d, p, _ -> d.pointOnCircle(p.elements[0], p.at, p.dofs.firstOrNull()) },
            ...
            ToolDef(POINT_ON_ELLIPSE, ToolCategory.POINTS, listOf(SlotKind.CONIC), replicates = false, icon = Icons.POINT_ON_ELLIPSE) { d, p, _ -> d.pointOnEllipse(p.elements[0], p.at, p.dofs.firstOrNull()) },
            ToolDef(POINT_ON_LINE, ToolCategory.POINTS, listOf(SlotKind.LINE), replicates = false, icon = Icons.POINT_ON_LINE) { d, p, _ -> d.pointOnLine(p.elements[0], p.at, p.dofs.firstOrNull()) },
            ToolDef(POINT_AT_DIST, ToolCategory.POINTS, listOf(SlotKind.POINT, SlotKind.LINE), scalars = listOf(len("distance")), icon = Icons.POINT_AT_DIST) { d, p, s -> d.pointAlongLine(p.elements[0], p.points[0], s[0], p.at) },
            ...
            ToolDef(POINT_XY, ToolCategory.POINTS, emptyList(), scalars = listOf(len("x"), len("y")), icon = Icons.POINT_XY) { d, _, s -> d.pointFromCoordinates(s[0], s[1]) },
            ...
            ToolDef(HEIGHT_POINT, ToolCategory.POINTS, listOf(SlotKind.POINT), scalars = listOf(len("height")), icon = Icons.HEIGHT_POINT) { d, p, s -> d.heightPointAt(p.points[0], s[0]) },
            ToolDef(CENTRE, ToolCategory.POINTS, listOf(SlotKind.CENTERED), icon = Icons.CENTRE) { d, p, _ -> d.centerOf(p.elements[0]) },
            ...
            ToolDef(PROJECT_TO_PLANE, ToolCategory.POINTS, listOf(SlotKind.POINT3, SlotKind.SIDE), crossSpace = true, recordsSteps = true, replicates = false, icon = Icons.PROJECT_TO_PLANE) { d, p, _ -> d.projectToPlane(p.elements[0]) },
            ...
            ToolDef(POINT_ON_HELIX, ToolCategory.POINTS, listOf(SlotKind.PATH3), replicates = false, icon = Icons.POINT_ON_HELIX) { d, p, _ -> d.pointOnHelix(p.elements[0], p.at, p.view, p.dofs.firstOrNull()) },
            ToolDef(KEY_POINTS, ToolCategory.POINTS, listOf(SlotKind.EXTRACTABLE), icon = Icons.KEY_POINTS) { d, p, _ -> d.extractPoints(p.elements[0]) },
            ...
            ToolDef(JOIN, ToolCategory.POINTS, listOf(SlotKind.EXISTING_POINT, SlotKind.EXISTING_POINT), replicates = false, icon = Icons.JOIN) { d, p, _ -> d.weld(p.elements[1], p.elements[0]) },
            ...
            ToolDef(MAKE_RELATIVE, ToolCategory.POINTS, listOf(SlotKind.EXISTING_POINT, SlotKind.INPUT_POINT), replicates = false, icon = Icons.MAKE_RELATIVE) { d, p, _ -> d.makeRelative(p.elements[0], p.elements[1], p.dofs) },
            ToolDef(MAKE_ABSOLUTE, ToolCategory.POINTS, listOf(SlotKind.EXISTING_POINT), replicates = false, icon = Icons.MAKE_ABSOLUTE) { d, p, _ -> d.makeAbsolute(p.elements[0], p.dofs) },
            ...
            ToolDef(UNLINK, ToolCategory.POINTS, listOf(SlotKind.EXISTING_POINT), replicates = false, fromSelection = true, icon = Icons.UNLINK) { d, p, _ -> d.unlink(p.elements[0], p.dofs) },
            // ----- Curves -----
            ToolDef(LINE, ToolCategory.CURVES, listOf(SlotKind.POINT, SlotKind.POINT), preview = Previews::line, icon = Icons.LINE) { d, p, _ -> d.line(p.points[0], p.points[1]) },
            ToolDef(SEGMENT, ToolCategory.CURVES, listOf(SlotKind.POINT, SlotKind.POINT), shortcut = 'L', preview = Previews::segment, icon = Icons.SEGMENT) { d, p, _ -> d.segment(p.points[0], p.points[1]) },
            ToolDef(RAY, ToolCategory.CURVES, listOf(SlotKind.POINT, SlotKind.POINT), preview = Previews::ray, icon = Icons.RAY) { d, p, _ -> d.ray(p.points[0], p.points[1]) },
            ToolDef(ORTHO_PATH, ToolCategory.CURVES, emptyList(), icon = Icons.ORTHO_PATH) { _, _, _ -> },
            ToolDef(BREAK_LEG, ToolCategory.CURVES, emptyList(), icon = Icons.BREAK_LEG) { _, _, _ -> },
            ToolDef(WALL, ToolCategory.CURVES, emptyList(), scalars = listOf(len("thickness")), shortcut = 'W', icon = Icons.WALL) { _, _, _ -> },
            ToolDef(OPENING, ToolCategory.CURVES, emptyList(), scalars = listOf(len("width")), shortcut = 'D', icon = Icons.OPENING) { _, _, _ -> },
            ToolDef(CIRCLE, ToolCategory.CURVES, listOf(SlotKind.POINT, SlotKind.POINT), preview = Previews::circleCentrePoint, icon = Icons.CIRCLE) { d, p, _ -> d.circle(p.points[0], p.points[1]) },
            ToolDef(CIRCLE_R, ToolCategory.CURVES, listOf(SlotKind.POINT), scalars = listOf(len("radius")), shortcut = 'C', icon = Icons.CIRCLE_R) { d, p, s -> d.circleCR(p.points[0], s[0]) },
            ToolDef(CIRCLE_3, ToolCategory.CURVES, listOf(SlotKind.POINT, SlotKind.POINT, SlotKind.POINT), preview = Previews::circle3, icon = Icons.CIRCLE_3) { d, p, _ -> d.circle3(p.points[0], p.points[1], p.points[2]) },
            ...
            ToolDef(CIRCLE_LLL, ToolCategory.CURVES, listOf(SlotKind.LINE, SlotKind.LINE, SlotKind.LINE, SlotKind.SIDE), preview = Previews::circle3Tangents, icon = Icons.CIRCLE_LLL) { d, p, _ -> d.circleFrom3Tangents(p.elements[0], p.elements[1], p.elements[2], p.at, p.signs) },
            ToolDef(ARC_3, ToolCategory.CURVES, listOf(SlotKind.POINT, SlotKind.POINT, SlotKind.POINT), preview = Previews::arc3, icon = Icons.ARC_3) { d, p, _ -> d.arc3(p.points[0], p.points[1], p.points[2]) },
            ToolDef(ARC_CS, ToolCategory.CURVES, listOf(SlotKind.POINT, SlotKind.POINT, SlotKind.POINT), preview = Previews::arcCentreEnds, icon = Icons.ARC_CS) { d, p, _ -> d.arcCenterStartEnd(p.points[0], p.points[1], p.points[2]) },
            ...
            ToolDef(ELLIPSE, ToolCategory.CURVES, listOf(SlotKind.POINT, SlotKind.POINT, SlotKind.POINT), preview = Previews::ellipse, icon = Icons.ELLIPSE) { d, p, _ -> d.ellipse(p.points[0], p.points[1], p.points[2]) },
            ToolDef(ELLIPSE_AB, ToolCategory.CURVES, listOf(SlotKind.POINT, SlotKind.POINT), scalars = listOf(len("semi-axis")), preview = Previews::ellipseAB, icon = Icons.ELLIPSE_AB) { d, p, s -> d.ellipseCAB(p.points[0], p.points[1], s[0]) },
            ToolDef(ELLIPTIC_ARC, ToolCategory.CURVES, listOf(SlotKind.POINT, SlotKind.POINT, SlotKind.POINT, SlotKind.POINT), scalars = listOf(len("semi-axis")), preview = Previews::ellipticArc, icon = Icons.ELLIPTIC_ARC) { d, p, s -> d.ellipticArc(p.points[0], p.points[1], s[0], p.points[2], p.points[3]) },
            ToolDef(BEZIER, ToolCategory.CURVES, listOf(SlotKind.POINT, SlotKind.POINT, SlotKind.POINT, SlotKind.POINT), icon = Icons.BEZIER) { d, p, _ -> d.bezierCurve(p.points[0], p.points[1], p.points[2], p.points[3]) },
            ...
            ToolDef(CURVE3, ToolCategory.CURVES, listOf(SlotKind.POINT3), repeating = true, minPicks = 2, crossSpace = true, closesOnFirstPick = true, icon = Icons.CURVE3) { d, p, _ -> d.curveThroughPoints(p.elements, smooth = false) },
            ToolDef(CURVE3_SMOOTH, ToolCategory.CURVES, listOf(SlotKind.POINT3), repeating = true, minPicks = 2, crossSpace = true, closesOnFirstPick = true, icon = Icons.CURVE3_SMOOTH) { d, p, _ -> d.curveThroughPoints(p.elements, smooth = true) },
            ...
            ToolDef(HELIX_PT, ToolCategory.CURVES, listOf(SlotKind.POINT3, SlotKind.POINT3), scalars = listOf(len("pitch"), num("turns", 1.0)), preview = Previews::helixBase, icon = Icons.HELIX_PT) { d, p, s -> d.helixThrough(p.elements[0], p.elements[1], s[0], s.getOrNull(1), Handedness.RIGHT) },
            ToolDef(HELIX_PT_LEFT, ToolCategory.CURVES, listOf(SlotKind.POINT3, SlotKind.POINT3), scalars = listOf(len("pitch"), num("turns", 1.0)), preview = Previews::helixBase, icon = Icons.HELIX_PT_LEFT) { d, p, s -> d.helixThrough(p.elements[0], p.elements[1], s[0], s.getOrNull(1), Handedness.LEFT) },
            ToolDef(HELIX, ToolCategory.CURVES, listOf(SlotKind.POINT3), scalars = listOf(len("radius"), len("pitch"), num("turns", 1.0)), icon = Icons.HELIX) { d, p, s -> d.helixAbout(p.elements[0], s[0], s[1], s.getOrNull(2), Handedness.RIGHT) },
            ToolDef(HELIX_LEFT, ToolCategory.CURVES, listOf(SlotKind.POINT3), scalars = listOf(len("radius"), len("pitch"), num("turns", 1.0)), icon = Icons.HELIX_LEFT) { d, p, s -> d.helixAbout(p.elements[0], s[0], s[1], s.getOrNull(2), Handedness.LEFT) },
            ...
            ToolDef(COMBINE_VIEWS, ToolCategory.CURVES, listOf(SlotKind.CURVE, SlotKind.CURVE), crossSpace = true, icon = Icons.COMBINE_VIEWS) { d, p, _ -> d.combineViews(p.elements[0], p.elements[1]) },
            ...
            ToolDef(INTERSECTION_CURVE, ToolCategory.CURVES, listOf(SlotKind.SECTION_CURVE), icon = Icons.INTERSECTION_CURVE) { d, p, _ -> d.intersectionCurve(p.elements[0], p.at, p.signs.firstOrNull()) },
            ...
            ToolDef(SPHERE_LOCUS, ToolCategory.CONSTRUCT, listOf(SlotKind.POINT3), scalars = listOf(len("radius")), crossSpace = true, icon = Icons.SPHERE_LOCUS) { d, p, s -> d.sphereLocus(p.elements[0], s[0]) },
            ToolDef(SPHERE_LOCUS_PT, ToolCategory.CONSTRUCT, listOf(SlotKind.POINT3, SlotKind.POINT3), crossSpace = true, preview = Previews::sphereLocusThrough, icon = Icons.SPHERE_LOCUS_PT) { d, p, _ -> d.sphereLocusThrough(p.elements[0], p.elements[1]) },
            ToolDef(SPHERE_CIRCLE, ToolCategory.CURVES, listOf(SlotKind.SPHERE, SlotKind.SPHERE), crossSpace = true, icon = Icons.SPHERE_CIRCLE) { d, p, _ -> d.sphereCircle(p.elements[0], p.elements[1]) },
            ToolDef(SPHERE_TRILATERATE, ToolCategory.POINTS, listOf(SlotKind.SPHERE, SlotKind.SPHERE, SlotKind.SPHERE, SlotKind.SIDE), crossSpace = true, icon = Icons.SPHERE_TRILATERATE) { d, p, _ -> d.trilateratePoint(p.elements[0], p.elements[1], p.elements[2], p.at, p.view, p.signs.firstOrNull()) },
            ToolDef(SPHERE_ON_RUN, ToolCategory.POINTS, listOf(SlotKind.SPHERE, SlotKind.PATH3), crossSpace = true, icon = Icons.SPHERE_ON_RUN) { d, p, _ -> d.sphereOnRun(p.elements[0], p.elements[1], p.at, p.view, p.signs.firstOrNull()) },
            ...
            ToolDef(LIFT, ToolCategory.CURVES, listOf(SlotKind.DRAWN_RUN), repeating = true, minPicks = 1, closesOnFirstPick = true, replicates = false, icon = Icons.LIFT) { d, p, _ -> d.liftCurves(p.elements) },
            ...
            ToolDef(CONNECT, ToolCategory.CURVES, listOf(SlotKind.PATH3, SlotKind.PATH3), crossSpace = true, scalars = listOf(num("tension", 1.0), num("far tension", 1.0)), scalarsTypedOnly = true, icon = Icons.CONNECT) { d, p, s -> d.connectCurves(p.elements[0], p.elements[1], s.getOrNull(0), s.getOrNull(1), p.clicks, p.signs, Continuity.G1) },
            ...
            ToolDef(PROJECT_ON_FACE, ToolCategory.CURVES, listOf(SlotKind.CURVE, SlotKind.SOLID), crossSpace = true, icon = Icons.PROJECT_ON_FACE) { d, p, _ -> d.projectOntoFace(p.elements[0], p.elements[1], p.signs) },
            ToolDef(CONNECT_G2, ToolCategory.CURVES, listOf(SlotKind.PATH3, SlotKind.PATH3), crossSpace = true, scalars = listOf(num("tension", 1.0), num("far tension", 1.0)), scalarsTypedOnly = true, icon = Icons.CONNECT_G2) { d, p, s -> d.connectCurves(p.elements[0], p.elements[1], s.getOrNull(0), s.getOrNull(1), p.clicks, p.signs, Continuity.G2) },
            ToolDef(OUTLINE, ToolCategory.RESULT, listOf(SlotKind.CURVE), repeating = true, followsBoundary = true, shortcut = 'O', icon = Icons.OUTLINE) { d, p, _ -> d.buildOutline(p.elements, p.clicks) },
            ToolDef(THICKEN, ToolCategory.RESULT, listOf(SlotKind.CURVE), scalars = listOf(len("thickness")), repeating = true, minPicks = 1, sidePerPick = true, replicates = false, extendsResult = true, preview = Previews::thicken, icon = Icons.THICKEN) { d, p, s -> d.buildThickNetwork(p.elements, p.signs.map { Tools.sideOf(it) }, s[0]) },
            ToolDef(CONCENTRIC, ToolCategory.CURVES, listOf(SlotKind.CIRCLE, SlotKind.SIDE), scalars = listOf(len("distance")), icon = Icons.CONCENTRIC) { d, p, s -> d.concentricCircle(p.elements[0], s[0], p.at) },
            ...
            ToolDef(RECTANGLE, ToolCategory.CURVES, listOf(SlotKind.SIDE, SlotKind.SIDE), shortcut = 'R', recordsSteps = true, preview = Previews::rectangle, icon = Icons.RECTANGLE) { d, p, _ -> d.orthoRectangle(p.clicks[0], p.clicks[1], p.landings.getOrNull(0), p.landings.getOrNull(1)) },
            ToolDef(ROUNDED_RECT, ToolCategory.CURVES, listOf(SlotKind.POINT, SlotKind.POINT), scalars = listOf(len("radius")), preview = Previews::roundedRect, icon = Icons.ROUNDED_RECT) { d, p, s -> d.roundedRectangle(p.points[0], p.points[1], s[0]) },
            ...
            ToolDef(POLYGON, ToolCategory.CURVES, listOf(SlotKind.POINT, SlotKind.POINT), scalars = listOf(choiceLen("corner radius", 0.0)), minCount = 3, recordsSteps = true, preview = Previews::polygon, icon = Icons.POLYGON) { d, p, s -> d.regularPolygonGesture(p, s) },
            ...
            ToolDef(EXTRUDE, ToolCategory.SOLIDS, listOf(SlotKind.AREA), scalars = listOf(len("depth")), shortcut = 'E', icon = Icons.EXTRUDE) { d, p, s -> d.extrudeSolid(p.elements[0], s[0]) },
            ...
            ToolDef(REVOLVE, ToolCategory.SOLIDS, listOf(SlotKind.AREA, SlotKind.LINE), scalars = listOf(angChoice("angle", 360.0), ang("offset", 0.0, typedOnly = true)), icon = Icons.REVOLVE) { d, p, s -> d.revolveSolid(p.elements[0], p.elements[1], s.getOrNull(0), s.getOrNull(1)) },
            ...
            ToolDef(SPHERE_R, ToolCategory.SOLIDS, listOf(SlotKind.POINT), scalars = listOf(len("radius")), icon = Icons.SPHERE_R) { d, p, s -> d.sphereCR(p.points[0], s[0]) },
            ToolDef(SPHERE, ToolCategory.SOLIDS, listOf(SlotKind.POINT, SlotKind.POINT), preview = Previews::circleCentrePoint, icon = Icons.SPHERE) { d, p, _ -> d.sphereCP(p.points[0], p.points[1]) },
            ...
            ToolDef(EXTRUDE_TO_POINT, ToolCategory.SOLIDS, listOf(SlotKind.AREA, SlotKind.POINT), scalars = listOf(len("height")), preview = Previews::extrudeToPoint, icon = Icons.EXTRUDE_TO_POINT) { d, p, s -> d.extrudeToPoint(p.elements[0], p.points[0], s[0], p.clicks.firstOrNull(), p.signs) },
            ToolDef(LOFT, ToolCategory.SOLIDS, listOf(SlotKind.LOFT_PART), repeating = true, minPicks = 2, crossSpace = true, preview = Previews::loft, icon = Icons.LOFT) { d, p, _ -> d.loftSolid(p.elements, p.clicks, p.signs) },
            ...
            ToolDef(TUBE, ToolCategory.SOLIDS, listOf(SlotKind.PATH3), scalars = listOf(len("radius"), ang("roll", 0.0), ang("twist", 0.0)), carriesLaw = true, icon = Icons.TUBE) { d, p, s -> d.tubeAlongCurve(p.elements[0], s[0], s.getOrNull(1), s.getOrNull(2), p.law) },
            ToolDef(SWEEP, ToolCategory.SOLIDS, listOf(SlotKind.PATH3, SlotKind.OPTIONAL_POINT, SlotKind.AREA), scalars = listOf(ang("roll", 0.0), ang("twist", 0.0)), crossSpace = true, carriesLaw = true, carriesLaws = true, icon = Icons.SWEEP) { d, p, s -> d.sweepAlongCurve(p.elements[0], p.elements[1], s.getOrNull(0), s.getOrNull(1), p.points.firstOrNull(), p.signs.firstOrNull(), p.law, p.laws) },
            ...
            ToolDef(EXTRUDE_ON_FACE, ToolCategory.SOLIDS, listOf(SlotKind.SOLID, SlotKind.AREA), scalars = listOf(len("depth")), icon = Icons.EXTRUDE_ON_FACE) { d, p, s -> d.extrudeOnFace(p.elements[0], p.elements[1], s[0]) },
            ToolDef(SECTION, ToolCategory.SOLIDS, listOf(SlotKind.SOLID), scalars = listOf(len("height")), icon = Icons.SECTION) { d, p, s -> d.sectionSolid(p.elements[0], s[0]) },
            ...
            ToolDef(CUT, ToolCategory.SOLIDS, listOf(SlotKind.AREA), scalars = listOf(len("depth")), facePartOperand = true, icon = Icons.CUT) { d, p, s -> d.cutOnFace(p.elements[0], p.elements[1], s[0]) },
            ...
            ToolDef(UNION, ToolCategory.SOLIDS, listOf(SlotKind.SOLID, SlotKind.SOLID), crossSpace = true, icon = Icons.UNION) { d, p, _ -> d.combineSolids(p.elements[0], p.elements[1], BoolOp.UNION) },
            ToolDef(SUBTRACT, ToolCategory.SOLIDS, listOf(SlotKind.SOLID, SlotKind.SOLID), shortcut = 'X', crossSpace = true, icon = Icons.SUBTRACT) { d, p, _ -> d.combineSolids(p.elements[0], p.elements[1], BoolOp.SUBTRACT) },
            ToolDef(INTERSECT_SOLIDS, ToolCategory.SOLIDS, listOf(SlotKind.SOLID, SlotKind.SOLID), crossSpace = true, icon = Icons.INTERSECT_SOLIDS) { d, p, _ -> d.combineSolids(p.elements[0], p.elements[1], BoolOp.INTERSECT) },
            ...
            ToolDef(BLEND_EDGE, ToolCategory.SOLIDS, listOf(SlotKind.SOLID), scalars = listOf(len("radius")), icon = Icons.BLEND_EDGE) { d, p, s -> d.blendEdges(p.elements[0], s[0], BlendKind.FILLET, whole = false, at = p.clicks.firstOrNull() ?: p.at, view = p.view, signs = p.signs) },
            ToolDef(BLEND_FACE, ToolCategory.SOLIDS, listOf(SlotKind.SOLID), scalars = listOf(len("radius")), icon = Icons.BLEND_FACE) { d, p, s -> d.blendEdges(p.elements[0], s[0], BlendKind.FILLET, whole = true, at = p.clicks.firstOrNull() ?: p.at, view = p.view, signs = p.signs) },
            ToolDef(CHAMFER_EDGE, ToolCategory.SOLIDS, listOf(SlotKind.SOLID), scalars = listOf(len("setback")), icon = Icons.CHAMFER_EDGE) { d, p, s -> d.blendEdges(p.elements[0], s[0], BlendKind.CHAMFER, whole = false, at = p.clicks.firstOrNull() ?: p.at, view = p.view, signs = p.signs) },
            ToolDef(CHAMFER_FACE, ToolCategory.SOLIDS, listOf(SlotKind.SOLID), scalars = listOf(len("setback")), icon = Icons.CHAMFER_FACE) { d, p, s -> d.blendEdges(p.elements[0], s[0], BlendKind.CHAMFER, whole = true, at = p.clicks.firstOrNull() ?: p.at, view = p.view, signs = p.signs) },
            ToolDef(PROFILE_EDGE, ToolCategory.SOLIDS, listOf(SlotKind.SOLID, SlotKind.BLEND_PROFILE), crossSpace = true, icon = Icons.PROFILE_EDGE) { d, p, _ -> d.blendEdges(p.elements[0], null, BlendKind.PROFILE, whole = false, at = p.clicks.firstOrNull() ?: p.at, view = p.view, signs = p.signs, profileEl = p.elements.getOrNull(1)) },
            ToolDef(PROFILE_FACE, ToolCategory.SOLIDS, listOf(SlotKind.SOLID, SlotKind.BLEND_PROFILE), crossSpace = true, icon = Icons.PROFILE_FACE) { d, p, _ -> d.blendEdges(p.elements[0], null, BlendKind.PROFILE, whole = true, at = p.clicks.firstOrNull() ?: p.at, view = p.view, signs = p.signs, profileEl = p.elements.getOrNull(1)) },
            ...
            ToolDef(LOFT_RULED, ToolCategory.SOLIDS, listOf(SlotKind.AREA), repeating = true, minPicks = 2, crossSpace = true, carriesMatches = true, icon = Icons.LOFT_RULED) { d, p, _ -> d.skinSolid(p.elements, SkinRow.RULED, p.matches) },
            ToolDef(LOFT_FAIRED, ToolCategory.SOLIDS, listOf(SlotKind.AREA), repeating = true, minPicks = 2, crossSpace = true, carriesMatches = true, icon = Icons.LOFT_FAIRED) { d, p, _ -> d.skinSolid(p.elements, SkinRow.FAIRED, p.matches) },
            ToolDef(MATCH_SECTIONS, ToolCategory.SOLIDS, listOf(SlotKind.CURVE, SlotKind.CURVE), replicates = false, crossSpace = true, icon = Icons.MATCH_SECTIONS) { d, p, _ -> d.matchSections(p.elements[0], p.elements[1]) },
            ToolDef(SHELL, ToolCategory.SOLIDS, listOf(SlotKind.SOLID), scalars = listOf(len("wall thickness")), icon = Icons.SHELL) { d, p, s -> d.shellSolid(p.elements[0], s[0], open = true, at = p.clicks.firstOrNull() ?: p.at, view = p.view, signs = p.signs) },
            ToolDef(SHELL_CLOSED, ToolCategory.SOLIDS, listOf(SlotKind.SOLID), scalars = listOf(len("wall thickness")), icon = Icons.SHELL_CLOSED) { d, p, s -> d.shellSolid(p.elements[0], s[0], open = false, at = p.clicks.firstOrNull() ?: p.at, view = p.view, signs = p.signs) },
            ...
            // named body (OP-23).
            ToolDef(PLACE_SOLID, ToolCategory.SOLIDS, listOf(SlotKind.SOLID, SlotKind.POINT), scalars = listOf(ang("angle", 0.0)), replicates = false, icon = Icons.PLACE_SOLID) { d, p, s -> d.placeSolid(p.elements[0], p.points[0], s.firstOrNull()) },
            ...
            ToolDef(PLACE_CURVE, ToolCategory.CURVES, listOf(SlotKind.PATH3, SlotKind.POINT), scalars = listOf(ang("angle", 0.0)), replicates = false, icon = Icons.PLACE_CURVE) { d, p, s -> d.placeCurve(p.elements[0], p.points[0], s.firstOrNull()) },
            ToolDef(CUT_OPENINGS, ToolCategory.SOLIDS, listOf(SlotKind.SOLID), icon = Icons.CUT_OPENINGS) { d, p, _ -> d.cutOpenings(p.elements[0]) },
            ...
            ToolDef(CHAIN, ToolCategory.CURVES, listOf(SlotKind.POINT), repeating = true, minPicks = 2, icon = Icons.CHAIN) { d, p, _ -> d.chainThroughPoints(p.points) },
            ToolDef(CUT_BY_CHAIN, ToolCategory.SOLIDS, listOf(SlotKind.SOLID, SlotKind.CHAIN, SlotKind.SIDE), crossSpace = true, icon = Icons.CUT_BY_CHAIN) { d, p, _ -> d.cutByChain(p.elements[0], p.elements[1], p.clicks.lastOrNull(), p.signs) },
            ToolDef(SPLIT_BY_CHAIN, ToolCategory.SOLIDS, listOf(SlotKind.SOLID, SlotKind.CHAIN), crossSpace = true, icon = Icons.SPLIT_BY_CHAIN) { d, p, _ -> d.splitByChain(p.elements[0], p.elements[1]) },
            ...
            ToolDef(CUT_ALONG_CURVE, ToolCategory.SOLIDS, listOf(SlotKind.SOLID, SlotKind.CHAIN, SlotKind.PATH3, SlotKind.SIDE), crossSpace = true, icon = Icons.CUT_ALONG_CURVE) { d, p, _ -> d.cutByChain(p.elements[0], p.elements[1], p.clicks.lastOrNull(), p.signs, p.elements[2], CarryMode.ROTATING) },
            ToolDef(CUT_ALONG_CURVE_FLAT, ToolCategory.SOLIDS, listOf(SlotKind.SOLID, SlotKind.CHAIN, SlotKind.PATH3, SlotKind.SIDE), crossSpace = true, icon = Icons.CUT_ALONG_CURVE_FLAT) { d, p, _ -> d.cutByChain(p.elements[0], p.elements[1], p.clicks.lastOrNull(), p.signs, p.elements[2], CarryMode.TRANSLATIONAL) },
            ToolDef(SPLIT_ALONG_CURVE, ToolCategory.SOLIDS, listOf(SlotKind.SOLID, SlotKind.CHAIN, SlotKind.PATH3), crossSpace = true, icon = Icons.SPLIT_ALONG_CURVE) { d, p, _ -> d.splitByChain(p.elements[0], p.elements[1], p.elements[2], CarryMode.ROTATING) },
            ToolDef(SPLIT_ALONG_CURVE_FLAT, ToolCategory.SOLIDS, listOf(SlotKind.SOLID, SlotKind.CHAIN, SlotKind.PATH3), crossSpace = true, icon = Icons.SPLIT_ALONG_CURVE_FLAT) { d, p, _ -> d.splitByChain(p.elements[0], p.elements[1], p.elements[2], CarryMode.TRANSLATIONAL) },
            ...
            ToolDef(PLANE_AT_HEIGHT, ToolCategory.PLANES, emptyList(), scalars = listOf(len("height")), recordsSteps = true, replicates = false, icon = Icons.PLANE_AT_HEIGHT) { d, _, s -> d.createParallelSpace(s[0]) },
            ...
            ToolDef(SKETCH_PLANE, ToolCategory.PLANES, listOf(SlotKind.LINE), scalars = listOf(ang("angle", 90.0), len("offset", 0.0)), recordsSteps = true, replicates = false, icon = Icons.SKETCH_PLANE) { d, p, s -> d.createDatumSpace(p.elements[0], s.firstOrNull(), offset = s.getOrNull(1)) },
            ...
            ToolDef(STATION, ToolCategory.PLANES, listOf(SlotKind.PATH3), scalars = listOf(len("distance")), recordsSteps = true, replicates = false, icon = Icons.STATION) { d, p, s -> d.createStationSpace(p.elements[0], s[0]) },
            ...
            ToolDef(SKETCH_FROM_WIRE, ToolCategory.PLANES, listOf(SlotKind.PATH3), recordsSteps = true, replicates = false, icon = Icons.SKETCH_FROM_WIRE) { d, p, _ -> d.sketchFromWireframe(p.elements[0]) },
            ...
            // boundary-piece index — a discrete choice, OP-18), so the Editor runs its click. **In the 3D
            // view the same click takes the face itself** (edit-in-3D slice 2): the ray resolves to a face
            // patch analytically and records the same address, so it is one behaviour with two gestures.
            ToolDef(SKETCH_ON_FACE, ToolCategory.PLANES, emptyList(), icon = Icons.SKETCH_ON_FACE) { _, _, _ -> },
            ...
            ToolDef(SPACE_ORIGIN, ToolCategory.PLANES, listOf(SlotKind.EXISTING_POINT), scalars = listOf(len("dx", 0.0), len("dy", 0.0)), replicates = false, scalarsTypedOnly = true, icon = Icons.SPACE_ORIGIN) { d, p, s -> d.setSpaceOriginAt(p.elements[0], s.getOrNull(0), s.getOrNull(1)) },
            // ----- Construct -----
            ...
            ToolDef(PERP_BISECTOR, ToolCategory.CONSTRUCT, listOf(SlotKind.POINT, SlotKind.POINT), scalars = listOf(choice("factor", 0.5)), icon = Icons.PERP_BISECTOR) { d, p, s -> d.perpBisector(p.points[0], p.points[1], s.firstOrNull()) },
            ToolDef(ANGLE_BISECTOR, ToolCategory.CONSTRUCT, listOf(SlotKind.POINT, SlotKind.POINT, SlotKind.POINT), icon = Icons.ANGLE_BISECTOR) { d, p, _ -> d.angleBisector(p.points[0], p.points[1], p.points[2]) },
            ToolDef(PERPENDICULAR, ToolCategory.CONSTRUCT, listOf(SlotKind.LINE, SlotKind.POINT), icon = Icons.PERPENDICULAR) { d, p, _ -> d.perpendicularThrough(p.elements[0], p.points[0]) },
            ToolDef(PARALLEL, ToolCategory.CONSTRUCT, listOf(SlotKind.LINE, SlotKind.POINT), icon = Icons.PARALLEL) { d, p, _ -> d.parallelThrough(p.elements[0], p.points[0]) },
            ToolDef(PARALLEL_AT, ToolCategory.CONSTRUCT, listOf(SlotKind.LINE, SlotKind.SIDE), scalars = listOf(len("distance")), icon = Icons.PARALLEL_AT) { d, p, s -> d.parallelAtDistance(p.elements[0], s[0], p.at) },
            ToolDef(TANGENT, ToolCategory.CONSTRUCT, listOf(SlotKind.POINT, SlotKind.CIRCLE), icon = Icons.TANGENT) { d, p, _ -> d.tangentFromPoint(p.points[0], p.elements[0]) },
            ...
            ToolDef(TANGENT_AT, ToolCategory.CONSTRUCT, listOf(SlotKind.ON_CIRCLE_POINT, SlotKind.CIRCLE), slotsNeeded = { d, els -> if (els.isNotEmpty() && d.circlesThrough(els[0]).size > 1) 2 else 1 }, icon = Icons.TANGENT_AT) { d, p, _ -> d.tangentAtPointOnCircle(p.elements[0], p.elements.getOrNull(1)) },
            ToolDef(FILLET, ToolCategory.CONSTRUCT, listOf(SlotKind.CARRIER, SlotKind.CARRIER), scalars = listOf(len("radius")), preview = Previews::fillet, icon = Icons.FILLET) { d, p, s -> d.filletBetweenCurves(p.elements[0], p.elements[1], s[0], p.clicks[0], p.clicks[1], p.signs) },
            // any carrier leg, since session 76: the chamfer-on-arc convention is decided — the setback is
            // measured **along the carrier**, so on a round leg it is an arc distance (see
            // [FilletMath.setback] for the argument, and DESIGN.md where the question was parked)
            ToolDef(CHAMFER, ToolCategory.CONSTRUCT, listOf(SlotKind.CARRIER, SlotKind.CARRIER), scalars = listOf(len("distance")), preview = Previews::chamfer, icon = Icons.CHAMFER) { d, p, s -> d.chamferBetweenCurves(p.elements[0], p.elements[1], s[0], p.clicks[0], p.clicks[1], p.signs) },
            ToolDef(OUTER_TANGENTS, ToolCategory.CONSTRUCT, listOf(SlotKind.CIRCLE, SlotKind.CIRCLE), icon = Icons.OUTER_TANGENTS) { d, p, _ -> d.commonTangents(p.elements[0], p.elements[1], inner = false) },
            ToolDef(INNER_TANGENTS, ToolCategory.CONSTRUCT, listOf(SlotKind.CIRCLE, SlotKind.CIRCLE), icon = Icons.INNER_TANGENTS) { d, p, _ -> d.commonTangents(p.elements[0], p.elements[1], inner = true) },
            // ----- Transform -----
            ToolDef(MIRROR, ToolCategory.TRANSFORM, listOf(SlotKind.GEOMETRY, SlotKind.LINE), preview = Previews::mirror, icon = Icons.MIRROR) { d, p, _ -> d.mirror(p.elements[0], p.elements[1]) },
            ...
            ToolDef(POINT_REFLECT, ToolCategory.TRANSFORM, listOf(SlotKind.GEOMETRY, SlotKind.POINT), preview = Previews::pointReflect, icon = Icons.POINT_REFLECT) { d, p, _ -> d.pointReflect(p.elements[0], p.points[0]) },
            ToolDef(ROTATE, ToolCategory.TRANSFORM, listOf(SlotKind.GEOMETRY, SlotKind.POINT), scalars = listOf(ang("angle")), preview = Previews::rotate, icon = Icons.ROTATE) { d, p, s -> d.rotate(p.elements[0], p.points[0], s[0]) },
            ToolDef(SCALE, ToolCategory.TRANSFORM, listOf(SlotKind.GEOMETRY, SlotKind.POINT), scalars = listOf(num("factor")), preview = Previews::scale, icon = Icons.SCALE) { d, p, s -> d.scale(p.elements[0], p.points[0], s[0]) },
            ToolDef(TRANSLATE_V, ToolCategory.TRANSFORM, listOf(SlotKind.GEOMETRY, SlotKind.POINT, SlotKind.POINT), preview = Previews::translate, icon = Icons.TRANSLATE_V) { d, p, _ -> d.translateByVector(p.elements[0], p.points[0], p.points[1]) },
            ...
            ToolDef(ARRAY_LINEAR, ToolCategory.TRANSFORM, listOf(SlotKind.GEOMETRY, SlotKind.POINT, SlotKind.POINT), minCount = 2, groupOperand = true, replicates = false, preview = Previews::linearArray, icon = Icons.ARRAY_LINEAR) { d, p, _ -> d.linearArray(p.elements, p.points[0], p.points[1], p.count) },
            ToolDef(ARRAY_CIRCULAR, ToolCategory.TRANSFORM, listOf(SlotKind.GEOMETRY, SlotKind.POINT), minCount = 2, groupOperand = true, replicates = false, preview = Previews::circularArray, icon = Icons.ARRAY_CIRCULAR) { d, p, _ -> d.circularArray(p.elements, p.points[0], p.count) },
            ...
            ToolDef(PATTERN_CIRCULAR, ToolCategory.TRANSFORM, listOf(SlotKind.POINT, SlotKind.POINT), minCount = 2, recordsSteps = true, preview = Previews::circularPattern, icon = Icons.PATTERN_CIRCULAR) { d, p, _ -> d.createPattern(PatternKind.CIRCULAR, p.points[1], p.points[0], p.count) },
            ToolDef(PATTERN_LINEAR, ToolCategory.TRANSFORM, listOf(SlotKind.POINT, SlotKind.POINT), minCount = 2, recordsSteps = true, preview = Previews::linearPattern, icon = Icons.PATTERN_LINEAR) { d, p, _ -> d.createPattern(PatternKind.LINEAR, p.points[0], p.points[1], p.count) },
            // ----- Measure -----
            ...
            ToolDef(DISTANCE, ToolCategory.MEASURE, listOf(SlotKind.POINT, SlotKind.POINT), replicates = false, icon = Icons.DISTANCE) { d, p, _ -> d.measureDistance(p.points[0], p.points[1]) },
            ToolDef(ANGLE, ToolCategory.MEASURE, listOf(SlotKind.POINT, SlotKind.POINT, SlotKind.POINT), replicates = false, icon = Icons.ANGLE) { d, p, _ -> d.measureAngle(p.points[0], p.points[1], p.points[2]) },
            ToolDef(LENGTH, ToolCategory.MEASURE, listOf(SlotKind.MEASURABLE), replicates = false, icon = Icons.LENGTH) { d, p, _ -> d.measureLength(p.elements[0]) },
            ToolDef(RADIUS, ToolCategory.MEASURE, listOf(SlotKind.CIRCLE), replicates = false, icon = Icons.RADIUS) { d, p, _ -> d.measureRadius(p.elements[0]) },
            ToolDef(COORD_X, ToolCategory.MEASURE, listOf(SlotKind.POINT), replicates = false, icon = Icons.COORD_X) { d, p, _ -> d.measureX(p.points[0]) },
            ToolDef(COORD_Y, ToolCategory.MEASURE, listOf(SlotKind.POINT), replicates = false, icon = Icons.COORD_Y) { d, p, _ -> d.measureY(p.points[0]) },
            ToolDef(ANGLE_LINES, ToolCategory.MEASURE, listOf(SlotKind.LINE, SlotKind.LINE), replicates = false, icon = Icons.ANGLE_LINES) { d, p, _ -> d.measureAngleLines(p.elements[0], p.elements[1]) },
            ...
            ToolDef(VOLUME, ToolCategory.MEASURE, listOf(SlotKind.SOLID), replicates = false, icon = Icons.VOLUME) { d, p, _ -> d.measureSolidVolume(p.elements[0]) },
            ToolDef(EXTENT_X, ToolCategory.MEASURE, listOf(SlotKind.SOLID), replicates = false, icon = Icons.EXTENT_X) { d, p, _ -> d.measureSolidExtent(p.elements[0], Axis3.X) },
            ToolDef(EXTENT_Y, ToolCategory.MEASURE, listOf(SlotKind.SOLID), replicates = false, icon = Icons.EXTENT_Y) { d, p, _ -> d.measureSolidExtent(p.elements[0], Axis3.Y) },
            ToolDef(EXTENT_Z, ToolCategory.MEASURE, listOf(SlotKind.SOLID), replicates = false, icon = Icons.EXTENT_Z) { d, p, _ -> d.measureSolidExtent(p.elements[0], Axis3.Z) },
            // ----- Annotate: dimensions (OP-4) — the graphic shows a measurement, and drives nothing -----
            ToolDef(DIM_LINEAR, ToolCategory.ANNOTATE, listOf(SlotKind.INPUT_POINT, SlotKind.INPUT_POINT, SlotKind.SIDE), shortcut = 'M', replicates = false, preview = Previews::linearDimension, icon = Icons.DIM_LINEAR) { d, p, _ -> d.linearDimension(p.elements[0], p.elements[1], p.at, p.dofs) },
            ToolDef(DIM_RADIAL, ToolCategory.ANNOTATE, listOf(SlotKind.CENTERED, SlotKind.SIDE), replicates = false, preview = Previews::radialDimension, icon = Icons.DIM_RADIAL) { d, p, _ -> d.radialDimension(p.elements[0], p.at, p.dofs) },
            ToolDef(DIM_ANGULAR, ToolCategory.ANNOTATE, listOf(SlotKind.LINE, SlotKind.LINE, SlotKind.SIDE), replicates = false, preview = Previews::angularDimension, icon = Icons.DIM_ANGULAR) { d, p, _ -> d.angularDimension(p.elements[0], p.elements[1], p.at, p.dofs) },
        )
    ...
    val legacy: List<ToolDef> =
        listOf(
            ToolDef(RECTANGLE_V1, ToolCategory.CURVES, listOf(SlotKind.POINT, SlotKind.POINT)) { d, p, _ -> d.rectangle(p.points[0], p.points[1]) },
        )
    ...
    fun roleOfKind(kind: SlotKind?): String = Messages.text("slot." + roleKeyOf(kind))

    /**
     * Which **word** a slot kind is said with (OP-29): a key into the message bundle rather than the word
     * itself, so several kinds that mean the same thing to a reader — every flavour of point, a curve and
     * an extractable curve — go on sharing one sentence in every language.
     */
    private fun roleKeyOf(kind: SlotKind?): String =
        when (kind) {
            null -> "input"
            SlotKind.PLACE_POINT, SlotKind.POINT, SlotKind.INPUT_POINT, SlotKind.EXISTING_POINT -> "point"
            SlotKind.OPTIONAL_POINT -> "ridePoint"
            SlotKind.ON_CIRCLE_POINT -> "pointOnCircle"
            SlotKind.CURVE, SlotKind.EXTRACTABLE -> "curve"
            SlotKind.LINE -> "line"
            SlotKind.CIRCLE, SlotKind.CENTRIC -> "circle"
            SlotKind.CONIC -> "ellipse"
            SlotKind.CENTERED -> "circleOrEllipse"
            ...
            SlotKind.BLEND_PROFILE -> "blendProfile"
            SlotKind.LOFT_PART -> "loftPart"
            SlotKind.POINT3 -> "point3"
            SlotKind.PATH3 -> "path3"
            SlotKind.DRAWN_RUN -> "drawnRun"
            SlotKind.SECTION_CURVE -> "sectionCurve"
            SlotKind.SPHERE -> "sphereLocus"
        }
